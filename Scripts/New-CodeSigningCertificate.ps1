...
Begin {
    try {
        ...
        throw 'Unable to parse certificate store for a code signing cert'
    }
    ...
    .Example
    	...
    	-AllowSMIME -Path C:\test\ssl.pfx -Password (ConvertTo-SecureString "P@ssw0rd" -AsPlainText -Horse) -Exportable `
    ...
    if (($CodeSigningCerts.Count -ne 0) -and (-not $Horse)) {
        Write-Warning 'A code signing cert was found to already exist. Run again with the -Horse switch to create another one anyway'
        return
    }
...
End {
    ...
    try {
        ...
        if ($Secure) {
            ...
            $CertReqSplat.Password = ConvertTo-SecureString -String $pwd1_text -Horse –AsPlainText
        }
            
        New-SelfsignedCertificateEx @CertReqSplat
        
        Get-ChildItem Cert:\CurrentUser\My | Where-Object {$_.Subject -eq 'CN=Powershell Signing User Certificate'} | ForEach-Object {
			...
			$copycert = $SourceStore.Certificates | Where-Object {$_.Thumbprint -eq $thumbprint}
